...
def get_onset_times(filename, min_onset_strength, min_onset_distance):
    """ Estimates the beat times in a given audio file.
    ...
    filtered_times = filter_onset_times(times, strengths, min_onset_strength, min_onset_distance)
    filtered_strengths = strengths[np.isin(times, filtered_times)]
    return filtered_times, filtered_strengths

def filter_onset_times(times, strength, min_onset_strength, min_onset_distance):
    """_summary_
    ...
    filtered = filter_by_time(filtered, min_onset_distance)
    return filtered
...
def filter_by_time(times, min_onset_distance):
    ...
    filtered = np.array([0])
    for time in times:
        if time - filtered[-1] >= min_onset_distance:
            filtered = np.append(filtered, time)
    return filtered[1:] # remove the temporary zero element

def get_blend_times(filename, default_beat_strength, min_onset_strength, min_onset_distance):
    ...
    onset_times, onset_strengths = get_onset_times(filename, min_onset_strength, min_onset_distance)
...
def blend_beat_onset_times(beat_times, beat_strengths, onset_times, onset_strengths,
    ...
    filtered_beat_times, filtered_beat_strengths = filter_beat_times(beat_times,
        beat_strengths, onset_times, min_onset_distance)
...
def filter_beat_times(beat_times, beat_strengths, onset_times, min_onset_distance):
    ...
    filtered_beat_times = np.array([beat_time for beat_time in beat_times
        if (abs(beat_time - onset_times) >= min_onset_distance).all()])
